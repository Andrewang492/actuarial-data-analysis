...
Your actuarial consulting firm has been hired by a large hospital ABC to help the hospital executives gain a better understanding of the factors that drive inpatient length of stay. This insight will allow the administrators to better understand and manage patient needs.

ABC has provided data on historical inpatient encounters for patients with diabetes. Each encounter includes the length of stay in hospital, measured in days. Your task is to use the available data to identify and interpret the factors that relate to inpatient stay duration.
...
**Question** Identify a variable that may have potential ethical concerns. Discuss the considerations related to using this variable in a model for this business problem. (Note: ethical considerations will be formally discussed in week 10, but it would be good to have some initial thoughts on this issue.)
...
**Question**: We do not know if the missing values of "race" variables are at random or not. what do you think and how would you deal with this missing value?

**Question**: I convert admit_type_id to a factor. Explain why.
...
**Question**: Regrouping the levels of the race variable. Asian, Hispanic, and Other will be in the same category. Explain why?
...
Releveling factor variables using the code below. The factor variable levels were reordered so that the most frequent level was first.
...
**Question**: Which variables would you select for modelling purposes later by investigating variables and the relationship to the target variable?
...
**Question**: Explain the process of using out-of-bag errors to select the hyper-parameter 'mtry'. What is the selected value of 'mtry' ?
...
**Question**: Explain the importance of the values of the two hyper-parameters "shrinkage parameter (learning rate)" and "number of trees" in GBM and how they may affect the modelling results.
...
**Question**: Which model will you choose based on this result?
...
We perform feature selection with lasso regression. We use cross-validation to determine appropriate level of lambda for lasso regression.

**Question** Explain the effect of lambda on the modelling outcome.
...
**Question**: Interpret the model for the client.
...
**Question**: What other measures you could use to compare the model performance? Discuss their advantages and limitations.
